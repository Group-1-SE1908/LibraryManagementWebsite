...
public class LibrarianBorrowService {
    ...
    // public BigDecimal[] returnBook(long borrowId, String inputBarcode) throws
    // SQLException {
    ...
    // try {
    // // 1. Lấy thông tin đối chiếu (Sử dụng TRIM trong SQL để loại bỏ khoảng trắng
    // ẩn
    // // trong DB)
    ...
    // // + "WHERE br.id = ?";
    //
    // String sqlCheck = "SELECT br.status, br.due_date, br.copy_id, br.book_id,
    // bc.barcode, "
    ...
    // + "WHERE br.id = ? AND br.status IN ('BORROWED', 'RECEIVED',
    // 'RETURN_REQUESTED')";
    ...
    // "Lỗi: Phiếu mượn này chưa được gán mã vạch (copy_id bị NULL hoặc không tồn
    // tại).");
    // }
    //
    // }
    ...
    // throw new IllegalArgumentException("Sai mã vạch! Phiếu này yêu cầu mã '" +
    // correctBarcode
    ...
    // String updateBr = "UPDATE borrow_records SET status='RETURNED',
    // return_date=GETDATE() WHERE id=?";
    ...
    //// notificationService.notifyDepositRefunded(borrowUserId, bookTitle,
    // depositRefund);
    //// } catch (Exception e) {
    //// System.err.println("[LibrarianBorrowService] Lỗi hoàn tiền cọc borrowId=" +
    // borrowId
    //// + ": " + e.getMessage());
    //// }
    //
    ...
    //// System.err.println("[LibrarianBorrowService] Lỗi xử lý reservation sau trả
    // sách bookId=" + bookId
    //// + ": " + e.getMessage());
    //// }
    //
    ...
    // depositAmount.multiply(retentionRate).add(shipFee);
    ...
    // if (depositRefund.compareTo(BigDecimal.ZERO) < 0) depositRefund =
    // BigDecimal.ZERO;
    //
    // try {
    ...
    //
    // walletService.creditWallet(borrowUserId, depositRefund, "DEPOSIT_REFUND",
    // note, "DEPOSIT-REFUND-" + borrowId);
    // notificationService.notifyDepositRefunded(borrowUserId, bookTitle,
    // depositRefund);
    // } catch (Exception e) {
    // System.err.println("[LibrarianBorrowService] Lỗi hoàn tiền: " +
    // e.getMessage());
    // }
    // }
    //
    ...
    // Các hằng số nên được khai báo ở đầu lớp LibrarianBorrowService hoặc trong
    // BorrowService
    ...
    public BigDecimal[] returnBook(long borrowId, String inputBarcode) throws SQLException {
        ...
        try (Connection c = DBConnection.getConnection()) {
            ...
            try {
                ...
                try (PreparedStatement ps = c.prepareStatement(sqlCheck)) {
                    ...
                    try (ResultSet rs = ps.executeQuery()) {
                        if (rs.next()) {
                            ...
                            throw new IllegalArgumentException(
                                    "Lỗi: Phiếu mượn không hợp lệ hoặc không ở trạng thái có thể trả.");
                        }
                    }
                }
                ...
                // 5. Tính tiền phạt nếu trễ hạn (trước khi commit để lưu vào DB)
                LocalDate returnDate = LocalDate.now();
                BigDecimal fineAmount = calculateFine(dueDate, returnDate);

                // 6. Cập nhật phiếu mượn thành công (bao gồm fine_amount để tracking)
                String updateBr = "UPDATE borrow_records SET status='RETURNED', return_date=GETDATE(), "
                        + "fine_amount=?, is_paid=0 WHERE id=?";
                try (PreparedStatement ps = c.prepareStatement(updateBr)) {
