...
<html lang="vi">
    ...
    <body>
                ...
                <c:when test="${historyTotalCopies > 0}">
                    <div class="history-stack">
                        <c:forEach items="${historyEntries}" var="entry">
                            ...
                            <c:set var="rec" value="${entry.record}" />
                            <c:set var="statusUpper" value="${fn:toUpperCase(rec.status)}" />
                            <c:set var="methodUpper" value="${fn:toUpperCase(rec.borrowMethod)}" />
                            ...
                            <article class="history-card" data-copy-index="${entry.copyIndex}"
                                ...
                                <div class="history-card__actions">
                                    <div class="history-card__action">
                                        <button class="btn primary js-show-order" type="button"
                                                ...
                                                data-deposit="<fmt:formatNumber value="${r.depositAmount != null ? r.depositAmount : 0}" pattern="#,##0" /> ₫"
                                                data-copy-index="${entry.copyIndex}">Xem đơn hàng
                                        </button>
                                    </div>
                                    <c:if test="${methodUpper == 'ONLINE' && (statusUpper == 'BORROWED' || statusUpper == 'RECEIVED' || statusUpper == 'SHIPPING')}">
                                        <button type="button" class="btn btn-sm btn-outline-warning" 
                                                onclick="confirmReturnOnline(${rec.id})">
                                            <i class="fas fa-shipping-fast"></i> Trả sách qua Ship
                                        </button>
                                    </c:if>  
                                    ...
                                    <c:if test="${fn:toUpperCase(r.status) == 'REQUESTED' && fn:toUpperCase(r.borrowMethod) != 'ONLINE'}">
                                        <div class="history-card__action">
                                            ...
                                            <article class="renewal-history-record">
                                                ...
                                                <div class="renewal-history-record__contact">
                                                    ...
                                                    <strong class="renewal-history-record__value">
                                                        <c:out value="${not empty renewal.contactName ? renewal.contactName : r.user.fullName}"
                                                               default="Không có tên"/> ·
                                                        <c:out value="${not empty renewal.contactPhone ? renewal.contactPhone : r.user.phone}"
                                                               default="Không có số"/>
            ...
            <div class="renew-modal__panel" role="dialog" aria-modal="true" aria-labelledby="renew-modal-title" tabindex="-1">
                ...
                <form class="renew-form" method="post" action="${pageContext.request.contextPath}/borrow/renew" data-renew-form>
                    ...
                    <label class="form-group">
                        ...
                        <input type="tel" name="contactPhone" readonly inputmode="tel" pattern="[0-9]{10,11}"
        ...
        <div class="renewal-history-modal" hidden data-renewal-history-modal>
            ...
            <div class="renewal-history-modal__panel" role="dialog" aria-modal="true" aria-labelledby="renewal-history-title"
                 tabindex="-1">
                <button type="button" class="order-modal__close" aria-label="Đóng lịch sử yêu cầu" data-renewal-history-close>
                    ×
                </button>
        ...
        <script>
            ...
            function confirmReturnOnline(id) {
                if (confirm('Bạn muốn gửi yêu cầu trả sách qua dịch vụ vận chuyển?')) {
                    window.location.href = '${pageContext.request.contextPath}/borrow/returnOnline?id=' + id;
                }
            }
        </script>
