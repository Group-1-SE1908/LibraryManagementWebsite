...
<c:set var="userName" value="${not empty currentUser && not empty currentUser.fullName ? currentUser.fullName : 'Guest'}" />
...
<header>
    <div class="container">
        ...
        <ul class="nav-menu">
            ...
            <li><a href="${pageContext.request.contextPath}/borrow"><fmt:message key="nav.my_books" /></a></li>
            <li><a href="${pageContext.request.contextPath}/history"><fmt:message key="nav.history" /></a></li>
            <li><a href="${pageContext.request.contextPath}/books"><fmt:message key="nav.catalog" /></a></li>
        </ul>
            ...
            <c:choose>
                ...
                <c:when test="${not empty currentUser}">
                    <div class="user-profile" id="userProfileBtn" onclick="toggleUserDropdown(event)">
                        <c:choose>
                            <%-- Xử lý hiển thị Avatar hoặc chữ cái đầu tên [cite: 47-53] --%>
                            <c:when test="${not empty currentUser.avatar && currentUser.avatar != 'null'}">
                                <img class="user-avatar" src="${pageContext.request.contextPath}/${currentUser.avatar}"
                                     alt="Avatar" 
                        ...
                        <%-- Dropdown Menu: Đã loại bỏ link Quản lý của thủ thư [cite: 54-59] --%>
                        <div class="user-dropdown" id="userDropdown">
                            <a href="${pageContext.request.contextPath}/profile">👤 <fmt:message key="nav.profile" /></a>
                            <a href="${pageContext.request.contextPath}/logout" class="logout">🚪 <fmt:message key="nav.logout" /></a>
                        </div>
                    </div>
                </c:when>
                ...
                <c:otherwise>
                    <a href="${pageContext.request.contextPath}/login" style="color: white; text-decoration: none; font-weight: 600;">
